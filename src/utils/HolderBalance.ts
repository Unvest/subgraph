...
export function updateHolderBalance(
  ...
  holderBalance.claimed = claimedBalanceOfCall.reverted ? BigInt.fromI32(0) : claimedBalanceOfCall.value

  // Claimable Balance
  const claimableBalanceOfCall = vestingToken.try_claimableBalanceOf(userAddress)
  holderBalance.claimable = claimableBalanceOfCall.reverted ? BigInt.fromI32(0) : claimableBalanceOfCall.value

  // Locked Balance
  const lockedBalanceOfCall = vestingToken.try_lockedBalanceOf(userAddress)
  holderBalance.locked = lockedBalanceOfCall.reverted ? BigInt.fromI32(0) : lockedBalanceOfCall.value
